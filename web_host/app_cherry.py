...
def index():
    ...
    data = AirData.query.filter(AirData.time >= cur_time - datetime.timedelta(hours=8)).filter(
        AirData.id % 8 == 0).order_by(db.asc(AirData.time)).all()
    # tick2 = time.time()
    names = ['CO2',
             'CH2O',
             'TVOC',
    ...
    # tock = time.time()
    # logger.info(f'query {(tick2 - tick1) * 1000.}ms, other {(tock - tick2) * 1000.}ms')
...
def log_the_request():
    ...
    # logger.info(str(dir(request)))
    # logger.info(str(request.base_url))


if __name__ == '__main__':
    dispatcher = PathInfoDispatcher({'/': app})
    server = WSGIServer(('0.0.0.0', int(args.port)), dispatcher)
    server.start()
